...
from layers.MonarchMatrix import MonarchFFN
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, d_inner, n_head, dropout=0.1, normalize_before=True, seq_len=96, use_monarch=False):
        ...
        self.slf_attn = AttentionLayer(
            ...
            d_model, n_head, n_queries=seq_len, n_keys=seq_len,
            use_monarch=self.use_monarch)

        self.pos_ffn = PositionwiseFeedForward(
            d_model, d_inner, dropout=dropout, normalize_before=normalize_before, use_monarch=self.use_monarch)
...
class Encoder(nn.Module):
    ...
    def __init__(self, configs, window_size, inner_size, use_monarch=False):
        super().__init__()
        ...
        self.layers = nn.ModuleList([
            EncoderLayer(configs.d_model, configs.d_ff, configs.n_heads, dropout=configs.dropout,
                         normalize_before=False, seq_len=sum(self.all_size), use_monarch=use_monarch) for _ in range(configs.e_layers)
...
class PositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, d_in, d_hid, dropout=0.1, normalize_before=True, use_monarch=False):
        super().__init__()
        self.use_monarch = use_monarch
        self.normalize_before = normalize_before

        if self.use_monarch:
            self.ffn = MonarchFFN(d_in, dropout=dropout, activation="gelu")
        else:
