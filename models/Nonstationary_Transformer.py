...
class Model(nn.Module):
    ...
    def __init__(self, configs):
            ...
            [
                EncoderLayer(
                    AttentionLayer(
                        ...
                        n_queries=configs.seq_len, n_keys=configs.seq_len, use_monarch=self.use_monarch),
                    configs.d_model,
                    configs.d_ff,
        ...
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            ...
            #                             output_attention=False),
            #                 configs.d_model, configs.n_heads, n_queries=configs.label_len + configs.pred_len,
            #                 n_keys=configs.label_len + configs.pred_len, use_monarch=False),
            #             configs.d_model,
            #             configs.d_ff,
            #             dropout=configs.dropout,
            #             activation=configs.activation,
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # dec_out = dec_out * (std_enc[:, 0, :].unsqueeze(1).repeat(1, self.pred_len, 1))
        # dec_out = dec_out + (mean_enc[:, 0, :].unsqueeze(1).repeat(1, self.pred_len, 1))
        return dec_out
